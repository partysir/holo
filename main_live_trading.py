"""
main_live_trading.py - 实盘交易版 (修复版)
...
def reconcile_positions_after_orders(current_positions, orders_df):
    ...
    :param current_positions: 当前持仓 {stock: shares}
    :param orders_df: 交易订单DataFrame
    :return: 更新后的持仓字典
    """
...
def save_trading_orders(orders_df, output_dir='./live_trading'):
    ...
    for _, order in orders_df.iterrows():
        if order['action'] == 'buy':
            simple_orders.append(f"买入 {order['stock']} {int(order['shares'])}股")
        elif order['action'] == 'sell':
            simple_orders.append(f"卖出 {order['stock']} {int(order['shares'])}股")
...
def main():
    ...
    if len(orders) > 0:
        ...
        # ✅ 修复：使用 reconcile_positions_after_orders 更新持仓
